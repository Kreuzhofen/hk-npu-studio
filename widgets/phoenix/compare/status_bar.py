...
class CompareStatusBar(WorkspaceStatusBar):
    ...
    def _build(self, items: dict[str, str]) -> None:
        ...
        self.add_item("Original", items.get("Original", tr("not_loaded", "Nicht geladen")), 0)
        self.add_divider(1)
        self.add_item("Output", items.get("Output", tr("not_loaded", "Nicht geladen")), 2)
        self.add_divider(3)
        self.add_item("Zoom", items.get("Zoom", "Fit"), 4)
        self.add_divider(5)
        self.add_item("Sync", items.get("Sync", tr("ready", "Bereit")), 6)
        self.add_divider(7)
        self.add_item("Status", items.get("Status", tr("ready", "Bereit")), 8, sticky="e")
