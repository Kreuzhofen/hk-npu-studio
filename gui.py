...
class SnapdragonAIStudio(tk.Tk):
    ...
    def _button(self, parent, text, command, bg=ACCENT):
        return tk.Button(parent, text=text, command=command, bg=bg, fg="white",
                         ...
                         padx=14, pady=8, font=("Segoe UI", 10, "bold"), cursor="hand2")
    ...
    def _build_ui(self):
        ...
        self._label(sidebar, "SnapdragonAI", 18, True, TEXT, PANEL).pack(anchor="w", padx=18, pady=(22, 2))
        self._label(sidebar, "Studio v1.1 Identity", 10, False, MUTED, PANEL).pack(anchor="w", padx=20, pady=(0, 24))
        ...
        self._button(sidebar, "Generate öffnen", self.open_generate, bg=GREEN).pack(fill="x", padx=12, pady=(10, 2))
        self._button(sidebar, "Plugin Manager", self.open_model_manager, bg=PURPLE).pack(fill="x", padx=12, pady=(6, 2))
        self._button(sidebar, "About", self.open_about, bg=PANEL_2).pack(fill="x", padx=12, pady=(6, 2))
        tk.Frame(sidebar, bg=PANEL).pack(fill="both", expand=True)
        hw = tk.Frame(sidebar, bg=PANEL_2)
        hw.pack(fill="x", padx=12, pady=12)
        self._label(hw, "Systemstatus", 10, True, TEXT, PANEL_2).pack(anchor="w", padx=10, pady=(10, 2))
        self._label(hw, "Snapdragon X", 9, False, MUTED, PANEL_2).pack(anchor="w", padx=10)
        self._label(hw, f"Plugins: {len(self.plugins)}", 9, False, MUTED, PANEL_2).pack(anchor="w", padx=10)
        self._label(hw, "NPU / QNN bereit ✔", 9, False, GREEN, PANEL_2).pack(anchor="w", padx=10)
        self._label(hw, "Created by Holger Kreuzhofen", 8, False, MUTED, PANEL_2).pack(anchor="w", padx=10, pady=(4, 10))
        ...
        self._label(topbar, "AI Generate kann jetzt Workflows an ComfyUI senden", 10, False, MUTED, BG).pack(side="left", padx=14, pady=(8, 0))
        tk.Label(topbar, textvariable=self.status_text, bg=PANEL_2, fg=GREEN, padx=12, pady=6,
                 font=("Segoe UI", 10, "bold")).pack(side="right")
        generate_card = tk.Frame(main, bg=PANEL)
        generate_card.pack(fill="x", padx=24, pady=(0, 14))
        self._label(generate_card, "Bildgenerierung", 12, True, TEXT, PANEL).pack(anchor="w", padx=16, pady=(14, 4))
        self._label(generate_card, "ComfyUI lokal starten, API-Workflow speichern und Prompt aus SnapdragonAI Studio senden.",
                    10, False, MUTED, PANEL).pack(anchor="w", padx=16, pady=(0, 10))
        self._button(generate_card, "AI Generate öffnen", self.open_generate, bg=GREEN).pack(anchor="w", padx=16, pady=(0, 14))
        plugin_card = tk.Frame(main, bg=PANEL)
        plugin_card.pack(fill="x", padx=24, pady=(0, 14))
        self._label(plugin_card, "Aktives NPU-Test-Plugin", 12, True, TEXT, PANEL).pack(anchor="w", padx=16, pady=(14, 4))
        active_text = "Kein Plugin gefunden"
        if self.current_plugin:
            active_text = f"{self.current_plugin.icon} {self.current_plugin.name}  ·  {self.current_plugin.engine}  ·  {self.current_plugin.status()}"
        self._label(plugin_card, active_text, 10, False, MUTED, PANEL).pack(anchor="w", padx=16, pady=(0, 14))
        file_card = tk.Frame(main, bg=PANEL)
        file_card.pack(fill="x", padx=24, pady=(0, 14))
        self._label(file_card, "Upscaler-Testbild auswählen", 11, True, TEXT, PANEL).pack(anchor="w", padx=16, pady=(14, 6))
        row = tk.Frame(file_card, bg=PANEL)
        row.pack(fill="x", padx=16, pady=(0, 14))
        self.image_entry = tk.Entry(row, textvariable=self.selected_image, bg=PANEL_2, fg=TEXT,
                                    insertbackground=TEXT, relief="flat", font=("Segoe UI", 10))
        self.image_entry.pack(side="left", fill="x", expand=True, ipady=8)
        self._button(row, "Auswählen", self.choose_image, bg=ACCENT).pack(side="left", padx=(8, 0))
        self.start_button = self._button(row, "Plugin starten", self.start_plugin, bg=GREEN)
        self.start_button.pack(side="left", padx=(8, 0))
        self._button(row, "Plugin Manager", self.open_model_manager, bg=PURPLE).pack(side="left", padx=(8, 0))
        ...
        self._label(left, "Input", 11, True, TEXT, PANEL).pack(anchor="w", padx=14, pady=(12, 6))
        self.original_canvas = tk.Label(left, text="Kein Bild ausgewählt", bg=PANEL_2, fg=MUTED, font=("Segoe UI", 11))
        self.original_canvas.pack(fill="both", expand=True, padx=14, pady=(0, 14))
        self._label(right, "Output", 11, True, TEXT, PANEL).pack(anchor="w", padx=14, pady=(12, 6))
        self.result_canvas = tk.Label(right, text="Noch kein Ergebnis", bg=PANEL_2, fg=MUTED, font=("Segoe UI", 11))
        ...
        self.progressbar = ttk.Progressbar(progress_row, variable=self.progress_value, maximum=100,
        ...
        self._button(buttons, "Output öffnen", self.open_last_result, bg=PANEL_2).pack(side="left", padx=8)
        self._button(buttons, "Vergleich öffnen", self.open_compare, bg=ACCENT).pack(side="left")
        self._button(buttons, "Output-Ordner", self.open_output, bg=PANEL_2).pack(side="left", padx=8)
        log_card = tk.Frame(bottom, bg=PANEL)
        log_card.pack(fill="x", pady=(12, 0))
        self._label(log_card, "Protokoll", 10, True, TEXT, PANEL).pack(anchor="w", padx=12, pady=(10, 4))
        self.log_text = tk.Text(log_card, height=6, bg="#0b0f14", fg="#dbeafe",
                                insertbackground=TEXT, relief="flat", wrap="word", font=("Consolas", 9))
    ...
    def choose_image(self):
        ...
        path = filedialog.askopenfilename(initialdir=str(INPUT_DIR), title="Bild auswählen",
    ...
    def start_plugin(self):
        if not self.current_plugin:
            messagebox.showerror("Kein Plugin", "Kein Plugin verfügbar.")
            return
        if not self.current_plugin.available:
            messagebox.showerror("Plugin nicht bereit", self.current_plugin.details())
            return
        ...
        self.status_text.set("Plugin läuft...")
    ...
    def _worker(self, image_path):
        try:
            output_path = self.current_plugin.run(Path(image_path), log=self.log, status=self.set_status,
                                                  progress=self.set_progress, percent=self.set_percent)
        ...
        try:
            while True:
                ...
                if kind == "log":
                    ...
                    self.log_text.insert("end", "\nFEHLER:\n" + text + "\n")
                    self.log_text.see("end")
                    messagebox.showerror("Fehler", "Beim Plugin-Lauf ist ein Fehler aufgetreten. Details stehen im Protokoll.")
        except queue.Empty:
            pass
        self.after(100, self._poll_log_queue)
