...
* **Branding Update:** The official HK NPU STUDIO corporate logo assets have been finalized. GitHub now automatically switches between light and dark logo variants using the HTML <picture> element, providing optimal visibility on GitHub Desktop and Mobile. The README header layout has been redesigned to place the logo side-by-side with the title (vertically centered, narrow spacing), creating a clean, modern brand representation while preserving generous whitespace. The incorrect dynamic "license not specified" badge has been replaced with the correct static MIT license badge in the same style and color.
...
* **Model Path Resolution Hotfix:** `ModelRepository` trennt Modelldefinitionen nun ausdrücklich von installierten Modelldateien. Neben einem in der Definition deklarierten Pfad werden die in `data/preferences.json` konfigurierte `models_dir` und `config.MODELS_DIR` modellbezogen geprüft. Jeder Kandidat, sein Existenz-/Verzeichnistatus und der erste Treffer werden unter `[MODEL PATH]` protokolliert. Dadurch erkennt der dist-Build `C:\SnapdragonAI\models\sdxl_base`, selbst wenn die gebündelte Definition noch `installed: false` und einen leeren Pfad enthält. 34 relevante Tests, vollständige Produktkompilierung, isolierter ARM64-dist-Build und Release-Smoke-Test sind erfolgreich. Inferenzlogik und Execution Provider bleiben unverändert.

* **CPU-SDXL-Diagnoseinstrumentierung:** Der reale SDXL-CPU-Pfad protokolliert nun rein beobachtend alle Pipelinephasen, ONNX-Session-Metadaten, tatsächliche Ein-/Ausgabeformen, jeden `Session.Run`-Aufruf, jeden Denoising-Schritt und jede Fortschrittsänderung mit Zeitstempel, Dauer, Thread, Provider und Modellpfad. Ein passiver Watchdog meldet lang laufende ONNX-Aufrufe alle 30 Sekunden, ohne sie abzubrechen oder zu beeinflussen. Inferenzlogik, Modelle, Pfade, Provider, Ergebnisse und Fortschrittssteuerung bleiben unverändert.

* **CPU Generation Hotfix:** Der produktive ONNX-Pfad ist jetzt strikt fail-closed. QNN-EPContext-Pakete werden bei CPU-Auswahl nicht mehr als vermeintlich kompatible SDXL-Pakete ausgeführt; fehlende oder fehlerhafte Text-Encoder-, UNet- und VAE-Komponenten erzeugen einen protokollierten Fehler ohne PNG. Zufällige Embeddings/UNet-Ausgaben, der prozedurale „Mock VAE Decoder“ und die „ONNX Alpha Fallback Generation“ wurden vollständig aus dem Produktionspfad entfernt. Das validierte SDXL-ONNX-Paket ist im Produktmodellwähler verfügbar und verwendet den realen Adapter `ONNX Runtime CPU`. Zwei reale SDXL-Läufe mit identischem Seed und unterschiedlichen Prompts verwendeten ausschließlich `CPUExecutionProvider`; sämtliche Mock-Flags waren `false`. Die Bilder besitzen unterschiedliche SHA-256-Werte und unterscheiden sich in 236.907 Pixeln. 35 relevante Tests, Produktkompilierung und der Audit von 704 identischen Lokalisierungsschlüsseln sind erfolgreich.
...
* **Localization Hotfix:** Interne Prompt-, Batch-, Worker-, Queue- und Lifecycle-Zustände verwenden sprachneutrale kanonische Werte. Eine zentrale Laufzeitlokalisierung normalisiert auch ältere deutsche, englische und spanische Statusvarianten und übersetzt sie erst an der UI-Grenze. Statusleiste, Plugin-/Jobkarten, Queue, Prompt-Inspector, Dashboard, Galerie und Vergleich zeigen dadurch Status, Phasen und Fehlervorsilben ausschließlich in der gewählten Sprache; Backend- und Modellnamen bleiben unverändert. Deutsch, Englisch und Spanisch besitzen 704 identische Schlüssel. Der vollständige App-Start sowie dynamische Statuswechsel wurden in allen drei Sprachen geprüft; 27 relevante Tests und 9 Subtests sowie die Produktkompilierung sind erfolgreich. Es wurden keine gemischten Bildschirme festgestellt.

* **Visual Localization Fix:** Die vollständige Anwendung startet isoliert in Deutsch, Englisch und Spanisch. Ein verschärfter statischer Audit erfasst nun auch positionale Dialogargumente, nachträgliche Widget-Konfigurationen sowie sichtbare Log-, Status- und Engine-Ergebnistexte. Alle neun Phoenix-Seiten, Haupt- und Kontextmenüs, Dialogpfade, Modellbeschreibungen, Status-Badges, Queue-/Batchzustände und die dreisprachige Inno-Setup-Definition wurden geprüft. Dabei gefundene Mischungen in View-Titeln, Startoverlay, KI-Generierung, ControlNet-Beschreibungen, Plugin-/Importdialogen sowie Download-, Modelllade- und Backendmeldungen wurden beseitigt. Deutsch, Englisch und Spanisch besitzen 699 identische Schlüssel; der visuelle Laufzeit-Audit meldet 0 gemischte Oberflächen und 0 fremdsprachige UI-Texte. 54 relevante Tests und 9 Subtests sowie die Produktkompilierung sind erfolgreich.

* **Release Polish – Final:** Die linke Phoenix-Navigation verwendet nun 10–15 % größere Beschriftungen und Icons sowie feste Bereichsfarben für Startseite (Snapdragon-Blau), KI-Generierung (Violett), Modellmanager (Grün), Galerie (Gelb), Vergleich (Türkis), Erweiterungen (helleres Dunkelgrau) und Einstellungen (Rot). Beim aktiven Menüpunkt übernehmen Text, Icon und Indikator die jeweilige Bereichsfarbe. Der visuelle Lokalisierungsvertrag umfasst 600 identische Schlüssel in Deutsch, Englisch und Spanisch; insbesondere KI-Generierung, Modellmanager, Status-Badges, Detailfelder und sämtliche Katalog-Modellbeschreibungen sind sprachrein. Die bereits abgeschlossenen Kontextmenüs, Löschfunktionen und der technische Bildvergleich wurden erneut regressionsgeprüft. 25 betroffene Tests sowie die Produktkompilierung sind erfolgreich; Codesignierung und Veröffentlichung wurden nicht durchgeführt.

* **Sprint 22 – Final Polish & UX Audit:** Deutsch, Englisch und das neu vollständig ergänzte Spanisch verwenden 577 identische, automatisch geprüfte Lokalisierungsschlüssel einschließlich Platzhaltern; die erreichbaren Produktoberflächen und der Inno-Setup-Installer werden auf nicht lokalisierte sichtbare Texte geprüft. Phoenix-Branding, Light-Theme-Settings-Icon, Kontraste und DPI-Verhalten wurden vereinheitlicht. Texteingaben besitzen ein gemeinsames Standard-Kontextmenü; letzte Generierungen lassen sich einzeln oder nach Bestätigung vollständig samt Sidecars löschen. Der Bildvergleich bleibt als technischer Metadatenvergleich erhalten und zeigt zusätzliche Bild-, Datei- und Generierungsunterschiede. 78 betroffene Tests und 9 Subtests, Produktkompilierung, ARM64-Build, Artefaktvalidierung sowie isolierte Installation, Startdiagnose und Deinstallation sind erfolgreich. Codesignierung und Veröffentlichung wurden nicht durchgeführt.

* **Release-Vorbereitung 2.0 RC1:** Die reproduzierbare ARM64-Buildkette erzeugt mit PyInstaller eine eigenständige Anwendung, bereinigt lokale Modell- und Cachezustände und schreibt veränderliche Laufzeitdaten eingefrorener Builds unter `%LOCALAPPDATA%`. Inno Setup 6.7.3 erzeugt einen benutzerbezogenen ARM64-Installer. Anwendung und Installer wurden auf Snapdragon-Hardware gebaut; portable Diagnose, unbeaufsichtigte Installation, installierte Diagnose und Deinstallation waren erfolgreich. PE-Architektur, Version, Paketinhalt und SHA-256-Manifest sind validiert; 248 Tests und 23 Subtests sowie die Produktkompilierung sind erfolgreich. Codesignierung und Veröffentlichung wurden nicht durchgeführt.

* **Sprint 21 – Release Candidate:** Ein fail-closed Anwendungsupdate-Service prüft HTTPS-Manifeste, semantisch neuere Versionen, ARM64-Kompatibilität, Windows-Installer und verpflichtende SHA-256-Werte und stellt Updates über den resumefähigen atomaren Downloadpfad bereit, ohne Installer selbst zu starten. Die Release-Identität ist auf `2.0 RC1` / `2.0.0-rc.1` gesetzt. Das kuratierte End-to-End-Gate für Start, Updates, Plugins, Modellladen/Inference/Freigabe, Output/Galerie/Compare und Release-Konfiguration umfasst 67 erfolgreiche Tests und 3 Subtests; der Produktcode kompiliert vollständig. RC1 ist code-seitig vorbereitet, aber nicht veröffentlicht; Hardwarelauf, Build, Signierung und Upload bleiben dokumentierte manuelle Release-Schritte.

* **Sprint 20 – Produktreife & Release I:** Die Vergleichsansicht verwendet den gemeinsamen Sidecar-Vertrag, liefert deterministische Metadatenunterschiede und enthält keine deaktivierten TODO-Aktionen mehr. Plugin-Manifeste werden strikt validiert, fehlerhafte Plugins isoliert, deaktivierte Plugins nicht geladen und Installationen über ein vollständiges Staging-Verzeichnis atomar übernommen. `release.json` ist die gemeinsame Quelle für Produkt-, Paket-, Build-, Codename- und ARM64-Werte; ein parametrisierter Inno-Setup-Vertrag samt validierendem Buildskript ist vorbereitet, ohne einen Release auszuführen. 20 relevante Compare-, Plugin-, Release-/Installer- und Metadatentests sind erfolgreich.

* **Sprint 19 – Galerie & Dateiverwaltung absichern:** Galerie-Loader und Asset-Index verwenden einen gemeinsamen Vertrag für flache und verschachtelte Sidecar-Metadaten. Bildimporte, „Speichern unter“ und zentrale Sidecar-Nachbearbeitung nutzen atomare Dateiersetzung; Bild und gleichnamiges Sidecar werden gemeinsam übernommen und veraltete Zielmetadaten entfernt. Metadatenänderungen lösen zuverlässig einen Galerie-Refresh aus, während Thumbnail-Anfragen und Cacheeinträge an Größe, Änderungszeit und Dateigröße gebunden sind. 30 relevante Galerie-, Asset-, Thumbnail-, Speicher- und Pipeline-Tests sowie 9 Subtests sind erfolgreich; die UI wurde nicht neu gestaltet.
