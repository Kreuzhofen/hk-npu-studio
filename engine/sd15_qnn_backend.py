...
logger = get_logger("StableDiffusion15QnnBackend")
    ...
    def generate(self, job: GenerationJob) -> GenerationResponse:
        ...
        if not output_json_path.exists():
            logger.error(
                ...
                process.returncode, cmd, project_root, "\n".join(worker_output) or "<empty>",
            )
            return GenerationResponse(
                success=False,
                status="PipelineError",
                message=tr("generation_worker_failed", "Die Generierung konnte nicht abgeschlossen werden. Details wurden protokolliert."),
                model_name=params.model_name
            )
        ...
        try:
            ...
            logger.exception(
                ...
                process.returncode, output_json_path, "\n".join(worker_output) or "<empty>",
            )
        ...
        if result_data.get("success") and image_path and Path(image_path).is_file():
            ...
            logger.error(
                ...
                process.returncode, result_data.get("message", "<missing>"), image_path,
            )
            return GenerationResponse(
                success=False,
                status="PipelineError",
