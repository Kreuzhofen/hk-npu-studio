...
### Hinzugefügt
* **Backend Discovery & Environment Detection (P-061 & P-061.1 & P-061.2 & P-061.3):** Einführung des `BackendDiscoveryService` und der Datenklasse `DiscoveryResult` zur automatischen OS-, Python-, ONNX- und Qualcomm QNN SDK-Erkennung. Umgestaltung des Model-Inspectors in ein scrollbares Panel (`Canvas` + `Scrollbar`). Feinschliff der Treeview-Spaltenbreiten im Model Manager zur optimalen Lesbarkeit ohne Text-Clipping (z. B. "Available for Download").
